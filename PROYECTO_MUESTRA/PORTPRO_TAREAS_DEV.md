# Port Pro (móvil) — Tareas para el desarrollador
...
| 1.6 | Sustituir mocks en **una pantalla piloto** por la **primera query real** a Supabase (perfil del usuario o listado mínimo de cargas del conductor). |
| 1.7 | Definir capa `lib/supabase/client.ts` y hooks (`useAuth`, `useProfile`) alineados al estilo del monorepo web, sin lógica sensible en cliente. |
| 1.8 | **CI y secretos:** workflow con `lint` + `tsc --noEmit` (p. ej. GitHub Actions, ver `docs/GitHub_Setup_Guide.md`); matriz escrita de que **nunca** entre `SUPABASE_SERVICE_ROLE_KEY` en la app y qué operaciones irían al BFF del Next.js si RLS no alcanza. |
...
| # | Tarea |
|---|--------|
| 2.1 | Mapear tablas/vistas Supabase para “loads” / asignaciones al conductor (nombres reales en el esquema actual). |
| 2.2 | Implementar listado de cargas asignadas con paginación o límites razonables; manejo de loading, error y retry. |
| 2.3 | Pantalla detalle de carga: datos maestros y estado; cruzar con `docs/DISPATCHER_API_ROUTES.md` donde aplique. |
| 2.4 | Integrar **pull-to-refresh** e invalidación de caché (p. ej. TanStack Query, como en `docs/driver_app_roadmap.md`). |
| 2.5 | Corregir deuda de **vibe coding**: componentes frágiles, `any`, estilos inconsistentes heredados del maquetado. |
| 2.6 | Tests **unitarios** de utilidades de formato (fechas, estados, etiquetas) y de mappers DB → UI. |
| 2.7 | Tests de **hooks** con datos mockeados de Supabase (MSW o mock del cliente). |
| 2.8 | Completar `docs/MOBILE_API.md`: qué lee la app directo de Supabase vs qué llamará a rutas `app/api/` del TMS. |
...
| 3.8 | Actualizar `HANDOFF_DEV.md` del cliente con los cambios respecto al maquetado inicial. |
...
| # | Tarea |
|---|--------|
| 4.1 | **Backend / TMS:** cerrar la brecha POD — hoy `POST` en `app/api/dispatcher/loads/[id]/documents/route.ts` solo admite `admin`/`dispatcher`. Elegir e implementar **(A)** ampliar ese POST al conductor asignado, **(B)** Storage + RLS + insert en `load_documents`, o **(C)** ruta dedicada; mantener reglas de tamaño (p. ej. 50 MB) alineadas al route actual. |
| 4.2 | **Móvil:** integrar **expo-image-picker** (o cámara) contra la vía elegida en la tarea 4.1; flujo de confirmación y cancelación. |
| 4.3 | Validar **tamaño máximo** y tipos MIME en cliente antes de subir; coherente con políticas del bucket y del TMS. |
...
| 8.3 | Plan de **rollback** y relación de **migraciones** Supabase tocadas por la app o el TMS para Port Pro. |
...
- **Mapa código → móvil** (exclusiones, gaps POD, panel conductor): ver **`PORTPRO_DOCUMENTACION.md` §3** tras la revisión del monorepo TigerHawk.
