...
import {
  parseLoadPodPreviewResponse,
  type LoadPodPreview,
} from '@/lib/loads/pod-preview';
import { tmsApiPath } from '@/lib/tms/client';
import {
  TmsMobileApiError,
  parseMobileApiErrorBody,
} from '@/lib/tms/mobile-api-error';
import { mobileLoadPodPath } from '@/lib/tms/mobile-api-routes';
import { resolveSupabaseAccessToken } from '@/lib/tms/resolve-access-token';
import { TmsStatusChangeError } from '@/lib/tms/errors';

export type FetchLoadPodParams = {
  loadId: string;
  accessToken?: string;
  fetchImpl?: typeof fetch;
};

export type FetchLoadPodResult =
  | { ok: true; preview: LoadPodPreview }
  | { ok: false; error: string; mobileError?: TmsMobileApiError };

/** GET canonical POD preview — TMS facts only (TASKS G.1). */
export async function fetchMobileLoadPod(
  params: FetchLoadPodParams,
): Promise<FetchLoadPodResult> {
  let url: string;
  try {
    url = tmsApiPath(mobileLoadPodPath(params.loadId));
  } catch (error) {
  ...
  if (!response.ok) {
    const mobileError =
      ...
      new TmsMobileApiError(
        `Could not load POD preview (HTTP ${response.status}).`,
        {
          code:
            response.status === 401
              ? 'UNAUTHORIZED'
  ...
  return parseLoadPodPreviewResponse(body);
}
