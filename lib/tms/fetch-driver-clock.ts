...
export type FetchDriverClockResult =
  | { ok: true; state: DriverClockState }
  | { ok: false; error: string; mobileError?: TmsMobileApiError };

/** GET `/api/mobile/driver/clock` — live duty state (TASKS I.2). */
export async function fetchMobileDriverClock(params?: {
  accessToken?: string;
  fetchImpl?: typeof fetch;
}): Promise<FetchDriverClockResult> {
  let url: string;
  try {
  ...
  if (!response.ok) {
    const mobileError =
      ...
      new TmsMobileApiError(
        `Could not load clock status (HTTP ${response.status}).`,
        {
          code:
            response.status === 401
              ? 'UNAUTHORIZED'
  ...
  return parseDriverClockGetResponse(body);
}
