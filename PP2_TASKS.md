# PP2 (mobile) — Task list (English)

This document is the **English translation** of:

- `PP2_TAREAS_CLIENTE.md` — client tasks (~1 week)
- `PP2_TAREAS_DEV.md` — developer tasks (8 weeks × 8 tasks)
...
**Goal of this phase:** have the **“PP2” mobile app** (product name) ready to kick off, with **initial design and UI markup** supported by Claude, **without** standing up a new backend or database (the **same Supabase project** as the TigerHawk TMS is reused).
...
4. **Define “PP2” identity**: on-screen name, Expo slug, primary/secondary colors, typography (even if provisional). Capture decisions in `DESIGN_TOKENS.md` or a README section.
...
7. **Mark up MVP screens with Claude**: layouts, navigation, visual components, empty and error states — **UI only** (data can be mocks/local JSON). If you mock **POD upload**, label it in the handoff as **“requires TMS API work”** (today server-side document `POST` only allows `admin`/`dispatcher`; the developer will extend the path — see `PP2_DOCUMENTACION.md` §3.4).
...
- **What is not PP2 mobile:** the **customer portal** (`app/portal/`) is a different role and product; do not mix shipper customer flows into the driver app.

If the client needs legal/brand alignment (“PP2” vs internal TigerHawk naming), document it in the mobile repo README.
...
| 8.3 | **Rollback** plan and list of **Supabase migrations** touched by the app or TMS for PP2. |
...
- **Code map → mobile** (exclusions, POD gap, driver panel): see **`PP2_DOCUMENTACION.md` §3** after the TigerHawk monorepo review.
