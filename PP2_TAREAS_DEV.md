...
**Formato (desde el cierre · mayo–jun 2026):** **Semanas 1–7** + bloque **Completadas** (post v0.1.0) = histórico **✅**. **Backlog 9.1–9.7 ✅ (27 jun 2026).** Tareas absorbidas/eliminadas: **8.2**, **WT.13**, **WT.16**, **WT.17**, **WT.18**, **WT.26** — ver notas al final.
...
| 7.7 | ✅ **Completada (3 jun 2026).** `docs/BACKLOG_V1_1_7_7.md` — push, mensajes, wait time, geofencing, E2E, P2 quick wins; **rastreo en vivo** → **Completadas § GPS 8.4–8.16** · cierre **9.6**; orden sugerido v1.1. Enlazado desde `DRIVER_TMS_CAPABILITIES_5_7.md` y README.                                                             |
| 7.8 | ↪ **Pendiente → 9.7.** Handoff al cliente: APK/build, env, este archivo actualizado. Deadline original **9 jun** — margen imprevistos.                                                                                                                                                                                                    |

---

## Completadas — post v0.1.0 (jun 2026)

Bloques posteriores al deadline **9 jun** ya entregados. Tareas abiertas → **§ Pendientes (9.1–9.7)** al final del archivo.
...
### Wait time — post feedback (11–25 jun 2026)
...
### GPS en vivo (Semana 8)

| #    | Tarea                                                                                                                                                                                                         |
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 8.3  | ✅ **Completada (3 jun 2026).** Arquitectura fase 0: `docs/GPS_LIVE_TRACKING_ARCHITECTURE.md`, `docs/TMS_DEV_REPOSITORY.md`, regla `.cursor/rules/tms-dev-repository.mdc`.                                   |
...
| 8.7  | ✅ **Completada (22 jun 2026).** Móvil: `lib/location/tracking-policy.ts` — intervalo 30–60 s, estados viaje activo, umbral metros, payload Supabase; tests `tracking-policy.test.ts`.                       |
...
## Pendientes — backlog activo (9.1–9.7) ✅ cerrado 27 jun 2026

**Renumeración (25 jun 2026):** tareas inconclusas previas + feedback Lucas. **Todas completadas 9.1–9.7.**

**Tareas absorbidas / eliminadas (histórico):** **8.2** → **8.7**; **8.10–8.11, 8.14–8.15** → fase 0 (fuera de tabla activa); **WT.13** → **WT.20**; **WT.16** → **WT.34**; **WT.17** → **WT.27**; **WT.18** → **WT.23** Samsara; **WT.26** → **WT.35**.

| #       | Tarea                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **9.1** | ✅ **Completada (26 jun 2026).** Paridad transiciones **In Transit** (feedback Lucas 25 jun): fallback `MOCK_LOAD_TRANSITIONS` ampliado con **Arrived At Pickup**, **Arrived To Hook Container**, **Arrived At Return Empty**; online sigue `/api/dispatcher/transitions`; tests `driver-actions`, `web-driver-panel-parity`. **Supabase:** SUPABASE no requiere cambios. |
| **9.2** | ✅ **Completada (26 jun 2026).** Picker tipo documento (**Driver** / **POD** / **Photo**) en `PodUploadSection`; `DRIVER_UPLOAD_DOCUMENT_TYPES` + `driver-document-types.ts`; upload hook acepta `documentType`. **Supabase:** SUPABASE no requiere cambios (misma tabla `load_documents`). |
| **9.3** | ✅ **Completada (26 jun 2026).** Permisos cámara/galería: `pick-load-photo.ts` retorna `permission_denied`; `ErrorBanner` + **Open Settings** (`media-permission.ts`). **Supabase:** SUPABASE no requiere cambios. |
| **9.4** | ✅ **Completada (26 jun 2026).** Cola offline `lib/offline/` — encola status + upload; `OfflineQueueProcessor` + banner; flush al reconectar. Notas TMS sin UI móvil aún no encoladas. **Supabase:** SUPABASE no requiere cambios. |
| **9.5** | ✅ **Completada (27 jun 2026).** Samsara live-ready en TMS dev: parse **GeofenceExit** v2, `resolve-geofence-load.ts`, webhook + `SAMSARA_API_TOKEN` ping, modos `mock_stub` / `webhook_only` / `live`. Docs `SAMSARA_GEOFENCE_SPIKE.md`, `QA_SAMSARA_GEOFENCE_MOCK.md` §B. **Supabase:** SUPABASE no requiere cambios. |
| **9.6** | ✅ **Completada (27 jun 2026).** Sign-off GPS fase 0: `docs/GPS_LIVE_TRACKING_SIGNOFF_8_17.md`; matriz G1–G9 en `QA_DRIVER_LIVE_TRACKING.md`; backlog 8.10–8.15 documentado como diferido. **Supabase:** migraciones ya aplicadas (8.x) — sin cambios nuevos. |
| **9.7** | ✅ **Completada (27 jun 2026).** Handoff cliente: `docs/CLIENT_HANDOFF_9_7.md`, `HANDOFF_DEV.md` actualizado, builds EAS (`MOBILE_BUILDS.md`, `EAS_CREDENTIALS_HANDOFF_7_5.md`). Checklist reunión §9 pendiente firma cliente. |

**Reglas wait time confirmadas (cliente):**

| Regla | Estado |
| ----- | ------ |
| A — Wait time solo inicio manual (**Check In**) | **WT.27** ✅ |
| B — Fin manual = **Check Out** | **WT.27** ✅ |
| C — Único auto-stop: e-POD firmado/enviado | **WT.28** ✅ |
| D — Emails `customers.email`: 45 / 60 / cierre + cron | **WT.29–32** ✅ |
| E — Sync offline cola local | **9.4 ✅** |

**Dependencias:** paridad Bearer TMS (**7.1** ✅); estados en `DriverActionBar`. Wait time **no depende** de GPS **8.x**.
...
| Tema                                  | Dónde                                                                                                                            |
| ------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| **Backlog activo (pendiente)**        | **9.1–9.7 ✅ (27 jun)** · handoff `docs/CLIENT_HANDOFF_9_7.md` · post-v0.1 → `docs/BACKLOG_V1_1_7_7.md` |
| Backlog v1.1 (push, mensajes, E2E, …) | `docs/BACKLOG_V1_1_7_7.md`                                                                                                       |
| **Cobro tiempo excedido (wait time)** | **WT.1–35 ✅** · Samsara geofence stop **9.5 ✅** |
| **Rastreo GPS en vivo (Supabase)**    | **8.3–8.16 ✅** · sign-off **9.6 ✅** · **≠ 9.5 Samsara** |
| Transiciones conductor ↔ TMS          | **9.1 ✅** · online `/api/dispatcher/transitions` |
| Entornos desplegados                  | **`docs/DEPLOYMENT_STATUS.md`** — TMS Netlify ✅ · Expo/EAS ✅ · no reabrir WT.19                                                |
| Wait time manual + emails (cliente)   | `RESPUESTAS_CLIENTE.md` §287+ · **WT.27–WT.35** · geofence **9.5 ✅**                                                               |
...
- **Tareas eliminadas del listado activo:** **8.2**, **WT.13**, **WT.16**, **WT.17**, **WT.18**, **WT.26** — contenido integrado en filas completadas o **§ Pendientes 9.x**.
- **Renumeración pendientes (25 jun):** **7.8→9.7**, **8.17→9.6**, **WT.23→9.5**, **DOC.1→9.2**, **DOC.2→9.3**, **OFF.2→9.4**, **STAT.1 (Lucas)→9.1**.
