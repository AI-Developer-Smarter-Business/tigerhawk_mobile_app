-- PP2 Semana 8 — task 8.4 / 8.5: live driver location on loads (additive only)
-- Safe for shared Supabase (prod TMS + mobile): ADD columns + NEW policy + trigger guard.
-- Does NOT DROP or replace "Staff read/update shipments" or "Drivers read own loads".
-- Apply: Supabase Dashboard → SQL Editor (copy from supabase/sql-editor/ sibling file).
